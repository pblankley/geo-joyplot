...
class Mapper:
    ...
    def get_plot_df(top_left, bot_right, api_key=None, n_vert=15, n_horiz=30):
        ...
        v_range, h_range = Mapper._make_ranges(top_left, bot_right, n_vert=n_vert, n_horiz=n_horiz)

        elev_list = []
        for lat in v_range:
            q = Mapper._get_locs(lat, h_range)
            res = Mapper._ping_elev_api(q, api_key)
            elev = Mapper._parse_elev_json(res)
            elev_list.append(elev)

        min_e = np.min(elev_list)
        list_of_df = []
        for lat,ele in zip(v_range, elev_list):
            samps = Mapper._sample_elev(h_range, ele, min_e)
